...
figure's axis label/title now follow the EXACT unit string you typed as
input (ULEN, BOXPHYS, UVEL, TPHYS) instead of a fixed cgs notation or a
magnitude-based heuristic. Example:
  BOXPHYS="500 km"  -> folder/files say "box500km", plot axes show "[km]"
  BOXPHYS="3.2 cm"  -> folder/files say "box3.2cm",  plot axes show "[cm]"
If you only set BOXCODE (no physical unit for the box itself), the axis/tag
unit falls back to whatever unit you gave ULEN, since that is what fixes
the physical scale of the box in that case.
...
def sanitize_tag(text: str) -> str:
    """Make a user-typed unit string ('3.2 cm', '3e10 cm/s') safe to use in
    a folder/file name: no spaces, no '/', no '^'."""
...
unit_length_str = f"{ulen_q.value:g}{ulen_q.unit}"        # e.g. "1cm" -- used for tag + fallback axis unit
...
# --- Box size and its DISPLAY unit follow exactly what the user typed ------
if "BOXPHYS" in os.environ:
    boxphys_q = env_quantity("BOXPHYS", "5200 cm", expected_dim="length")
    ...
    axis_unit_name = boxphys_q.unit          # <- axes shown in the SAME unit as BOXPHYS
else:
    box_width_code = float(os.environ.get("BOXCODE", 5200))
    box_width_phys_cgs = box_width_code * unit_length_phys_cgs
    box_width_str = f"boxcode{box_width_code:g}"
    axis_unit_name = ulen_q.unit             # <- no physical box unit given: fall back to ULEN's unit
...
source_rate_phys = float(os.environ.get("SRC", 1e20))

tphys_q = env_quantity("TPHYS", "5.2e-7 s", expected_dim="time")
...
# ============================================================================
# RUN TAG: now built from the EXACT unit strings you typed as input, not
# from a fixed cgs scientific notation. Changing ULEN/BOXPHYS/UVEL/TPHYS
# therefore changes the folder name AND every saved filename accordingly.
# ============================================================================
run_tag = (
    f"N{size_shape}"
...
print(f"  axis display unit     = {axis_unit_name} (follows your BOXPHYS/ULEN input)")
print(f"  time display unit     = {time_axis_unit} (follows your TPHYS input)")
print("=" * 70)
print(f"  ULEN input            = '{unit_length_str}'  -> {unit_length_phys_cgs:.6e} cm")
print(f"  BOX  input            = '{box_width_str}'    -> {box_width_phys_cgs:.6e} cm")
print(f"  UVEL input            = '{unit_velocity_str}' -> {unit_velocity_phys:.6e} cm/s")
print(f"  TPHYS input           = '{tphys_str}' -> {t_phys:.6e} s")
...
# x_HII is a generic named passive scalar: declared via passive_names, with
# n_cons big enough to hold it (5 active + 1 passive = 6). equationmanager.py
...
# ============================================================================
# STAR GENERATION: random positions, masses, ages, metallicities
# # ===========================================================================

# n_stars = 30
# rng = np.random.default_rng(42)  # seed fixe -> mêmes étoiles à chaque run
...
# # Positions dispersées dans la boîte, avec une marge pour éviter les bords
# # (l'injection gaussienne autour de chaque étoile a besoin d'espace autour d'elle).
...
# ============================================================================
# STROMGREN SPHERE TEST: milieu ambiant neutre uniforme + une source
# ionisante ponctuelle au centre. rho_ambient_code / p_ambient_code / n_H_cgs
# / T_ambient_K sont deja calcules plus haut dans le script.
# ============================================================================
sol_test = sol_test.at[idx_rho_local].set(rho_ambient_code)
sol_test = sol_test.at[idx_p_local].set(p_ambient_code)
sol_test = sol_test.at[0, center, center, center].set(1e-20)  # graine de photons RT
sol_test = sol_test.at[9].set(0.0)  # x_HII: milieu entierement neutre au depart
...
def fmt_t_phys():
    """Physical time formatted in the SAME unit as your TPHYS input."""
...
# ============================================================================
# STROMGREN SPHERE VALIDATION: rayon analytique vs front simule
# ============================================================================
def stromgren_alpha_B_cgs(T_K):
    """Coefficient de recombinaison case B, ajustement standard
    (Osterbrock/Spitzer), valide autour de T ~ 1e4 K."""
    return 2.59e-13 * (T_K / 1.0e4) ** -0.7


T_ambient_code = T_ambient_K / cu.Temp_cgs
alpha_B_code = stellar_force.caseB(T_ambient_code)
alpha_B_cgs = alpha_B_code * (cu.L_cgs**3 / cu.T_cgs)  # inverse de rate_coeff_to_code
...
if xHII_abs_idx is not None and field_test.shape[0] > xHII_abs_idx:
    ...
    if below_half.size > 0:
        ...
        print("  !! Ionization front has not reached x_HII < 0.5 anywhere in the box "
              "(box may be fully ionized, or t_phys too small).")
...
# ============================================================================
# GIF: evolution of x_HII across iterations
# ============================================================================
...
# ============================================================================
# GIF: evolution of ALL fields across iterations
# ============================================================================
# GIF_FRAMES = int(os.environ.get("GIF_FRAMES", 30))
# dt_frame_code = time_code / GIF_FRAMES

# gif_field_names = ["E_gamma", "Fx", "Fy", "Fz", "rho", "vx", "vy", "vz", "p", "x_HII"]
# DIVERGING_FIELDS = {"Fx", "Fy", "Fz", "vx", "vy", "vz"}   # can be negative -> centered colormap
# BOUNDED_FIELDS = {"x_HII"}                                 # fixed [0, 1] scale
...
#     stack = np.stack(frame_slices[name], axis=0)  # (GIF_FRAMES, N, N)
...
#     else:  # E_gamma, rho, p -- positive-definite fields
#         cmap = "hot"
...
# ============================================================================
# PLOT  (axis unit + time display follow your input units directly)
# ============================================================================ji
...
# ============================================================================
# Photon density in cm^-3, axis unit follows your input unit
# ============================================================================
...
# ============================================================================
# Per-field slices + CSV dumps (folder name already follows run_tag)
# ============================================================================
