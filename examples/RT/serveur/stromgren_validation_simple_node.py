...
SINGLE-NODE VERSION -- one Slurm task, 4 local GPUs on the SAME node.

    #SBATCH -N 1
    #SBATCH --ntasks=1
    #SBATCH --gres=gpu:4

Do NOT call jax.distributed.initialize() here: that API is only needed to
discover several JAX PROCESSES spread across DIFFERENT nodes (multi-host).
On a single node with a single process, JAX already sees all local GPUs
through CUDA_VISIBLE_DEVICES -- calling jax.distributed.initialize() in this
context makes JAX start an unnecessary coordination service, which either
restricts the visible local devices to 1 or fails with a
"Connection refused" gRPC error, exactly the symptom observed.

NCCL IS still required though: even on a single node, JAX's multi-GPU
collectives (halo exchange via ppermute, sharded array transfers, etc., see
hydro_core.py) go through NCCL as soon as more than one local device is
used. So the NCCL load-check below is kept -- only jax.distributed.initialize()
is removed.

    python stromgren_validation_simple_node.py       # default N = 32
    N=48 python stromgren_validation_simple_node.py
"""
import math
import os
import sys
...
# multiple LOCAL devices. JAX picks them up automatically from
...
# --- NCCL sanity check: still relevant for single-node multi-GPU collectives
import ctypes
try:
    lib = ctypes.CDLL("libnccl.so.2")
    print(f"NCCL loaded OK: {lib}", flush=True)
except OSError as e:
    print(f"NCCL LOAD FAILED: {e}", flush=True)
...
try:
    lib = ctypes.CDLL("libnccl.so.2")
    print(f"NCCL loaded OK: {lib}")
except OSError as e:
    print(f"NCCL LOAD FAILED: {e}")
# ---------------------------------------------------------------------------
