...
(`dt = cfl / (ndim * c / dx_o)`) use that value.

POSTULATE: in the code's unit system, the unit of length IS 1
(unit_length_code = 1.0), by construction. This means ONE CELL = ONE CODE
LENGTH UNIT. Everything physical (box size, cell size in cm, etc.) must be
derived FROM this postulate, not the other way around: we choose the
physical size dx_phys (cm) that this single code-length-unit corresponds
to, and CodeUnits.from_config encodes exactly that correspondence
(1 code length unit = dx_phys cm). With c_code = 1 the radiation front
then advances exactly 1 cell = 1 code length unit per unit of code time
...
# PHYSICAL SETUP
# ============================================================================
# POSTULATE (fixed, never derived): the unit of length in the code's unit
# system is 1. This is the solver's own convention (dx_o = 1.0), so it is
# NOT a free parameter -- it is the starting point of the whole unit system.
#
#     unit_length_code = 1.0     <-- 1 code length unit = 1 cell, always.
#
# Everything else is now built AROUND this postulate:
#   - box_width_code : size of the box expressed in code length units
#                       (= number of cells, since 1 cell = 1 code unit)
#   - box_width_phys : size of the box in cm (the only free physical input)
#   - dx_phys        : physical size (cm) that ONE code length unit maps to
#                       -> this is what we tell CodeUnits.from_config, i.e.
#                       "1 code length unit (=1) corresponds to dx_phys cm"

dx_o              = float(os.environ.get("DX_O", 10))  # unites de longueur code par cellule
size_shape        = int(os.environ.get("N", 256))
box_width_code    = size_shape * dx_o                     # taille boite en unites code
box_width_phys    = 3.2                                   # cm
dx_phys           = box_width_phys / box_width_code        # cm par unite de longueur code
...
# --- code units: built so that ONE code length unit = ONE CELL -----------
# CodeUnits.from_config is told: "unit_length_code (= 1, the postulate above)
# corresponds physically to dx_phys cm". This is the only place the
# postulate unit_length_code = 1 gets translated into a physical scale.
...
dx_cell_code      = dx_phys          # = 1, the solver's own postulate
...
print("=" * 70)
print(f"  dx_o                  = {dx_o:.6f}  (POSTULATE: = 1, 1 cell = 1 code unit)")
print(f"  box_width_code        = {box_width_code:.6f}  code length units (= N cells)")
print(f"  L_cgs                 = {cu.L_cgs:.6e} cm  (= 1 code length unit = 1 cell = dx_phys)")
...
# consistent with the postulate unit_length_code = 1.
cf_test = dh.ConvectiveFlux_Radiative_transfer(eq_test, solver_test, dh.PLM(limiter="VANLEER"),dx=dx_o)


stellar_force = StellarRadiationForce(
    escape_fraction=0.1,
    dx=dx_cell_code,               # = unit_length_code = 1: only used as cell volume, unused in "stromgren"
...
hydrosim_test = dh.hydro(n_super_step=10000, fluxes=[cf_test], forces=[stellar_force], dx=dx_o)
print("hydro.dx_o =", hydrosim_test.dx_o, "   flux.dx_o =", cf_test.dx_o)
assert hydrosim_test.dx_o == cf_test.dx_o, "dx_o desynchronise !"
print("hydrosim_test.dx_o =", hydrosim_test.dx_o, " cf.dx_o =", cf_test.dx_o,
      " cfl =", eq_test.cfl)
# print("expected dt_code   =", eq_test.cfl / (3.0 * light_speed_code / unit_length_code))
...
for th in [1e-3, 1e-6, 1e-10, 1e-15]:
    ...
    print(f"  thr {th:.0e} of peak -> radius {r:3d} code units = {r * dx_phys:.4e} cm")
print(f"  expected free-streaming radius = {c_cgs * t_phys / dx_phys:.1f} code units")
...
def compute_extent_phys(size_shape, box_width_phys=box_width_phys):
    """
    Convertit les indices de pixels [0, size_shape] (= [0, box_width_code]
    en unites de longueur code, puisque 1 pixel = 1 code length unit) en
    unites physiques : pixel 0 -> 0, dernier pixel -> box_width_phys.
...
ax.set_title(f"Photons/cell, t = {t_phys:.2e} s  (ct = {c_cgs*t_phys/dx_phys:.0f} code units)")
...
pos = E_slice[E_slice > 0]
im = ax.imshow(np.ma.masked_less_equal(E_slice, 0.0), origin="lower", cmap="hot")
ax.set_xlabel("y [code length unit]"); ax.set_ylabel("x [code length unit]")
ax.set_title(f"Photons/cell, t = {t_phys:.2e} s  (ct = {c_cgs*t_phys/dx_phys:.0f} code units)")
fig.colorbar(im, ax=ax, label="photons per cell")
plt.tight_layout()
...
# ============================================================================
# Conversion "photons par cellule (= par code length unit^3)" -> "photons par cm^3" :
#   n_gamma [cm^-3] = N_gamma [photons/cellule] / dx_phys^3 [cm^3]
# dx_phys est la taille physique (cm) d'UN code length unit (= 1 cellule).
...
ax.set_title(f"Photon number density, t = {t_phys:.2e} s  "
             f"(ct = {c_cgs*t_phys/dx_phys:.0f} code units)")
...
# ============================================================================
# Toutes les distances "r" ci-dessous sont en CODE LENGTH UNITS (1 cellule = 1
# unite de longueur code = unit_length_code), conformement au postulat.
...
def analyze_inverse_r2(field_3d, size_shape, cell_size_phys, tag,
    ...
    print(f"[{tag}] injection_radius={injection_radius}, fit_range=[{r_valid.min():.1f}, {r_valid.max():.1f}] code units")
    ...
    ax.set_xlabel("log(r [code length unit])")
    ...
    ax.set_xlabel("Cell index (center + r) [code length unit]")
