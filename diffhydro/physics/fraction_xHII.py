...
class HydrogenPhotoChemistryForce:
    ...
    def __init__(
        ...
        self.handles_photon_chemistry = True
        ...
        self.fixed_temperature_K = (
            None if fixed_temperature_K is None else float(fixed_temperature_K)
        )
        if self.case not in ("A", "B"):
            raise ValueError(f"case must be 'A' or 'B', got {case!r}")
        if self.mean_photon_energy_eV < hchem.E_HI_EV:
            raise ValueError(
                "mean_photon_energy_eV must be at least the 13.6 eV "
                "ionization threshold."
            )
        if self.fixed_temperature_K is not None:
            if self.fixed_temperature_K <= 0.0:
                raise ValueError("fixed_temperature_K must be > 0.")
            if self.include_heating or self.include_cooling:
                raise ValueError(
                    "fixed_temperature_K is an isothermal benchmark mode; "
                    "set include_heating=False and include_cooling=False."
                )
    ...
    #     absorbed_frac = jnp.minimum(-jnp.expm1(-k * dt_s) , self.max_frac)#jnp.minimum(-jnp.expm1(-k * dt_s), self.max_frac) #
    ...
    #     dN_cgs = -N_abs_eff + (S / k_safe) * absorbed_frac # nouvelle version du chat
    #     # dN_cgs = -N_abs_eff + S * dt_s # old version du chat 

    #     sol = view.add_photons_cgs(sol, dN_cgs)
    #     sol = view.set_xHII(sol, x_new)

    #     # flux sink, eq. (26'), then M1 cone
    #     decay = jnp.exp(-k * dt_s)#-k * dt_s#* dt_s #
    #     for j in view.idx_F:
    ...
    #     # Classical Stromgren Test 1 assumes a prescribed, spatially uniform
    #     # temperature.  Project *after* x_HII has changed so the conservative
    #     # energy also accounts for the newly created electron population.
    #     if self.fixed_temperature_K is not None:
    #         sol = view.set_temperature_K(
    #             sol, self.fixed_temperature_K, x_HII=x_new
    #         )
    def force(self, i, sol, params, dt):
        view = self.view
        dt_s = view.dt_cgs(dt)
        ...
        # --- RAMSES-RT SIE, eq. (A7), (A9), (A10) --------------------------
        D = n_HI * self.sigma_HI_cgs * c_cgs                    # s^-1, eq. (A7)
