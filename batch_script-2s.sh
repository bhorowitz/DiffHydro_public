...
#CUDA_VISIBLE_DEVICES=3 python cosmo_reconstruct/run_optimize_cv0_density_full_hydro.py --mesh-n 128 --z-init 127.0 --z-target 2.0 --hydro-steps 96 --solver hll --dual-energy --temperature0 20000 --rho-unit-cgs 1.6e-24 --enable-cooling --cooling-model nyx_table --nyx-cooling-table-npz diffhydro_gadgetic_n128_z127to2_hll/nyx_cooling_table.npz --n-iters 200 --nyx-cooling-z-nodes '2,3,4,5,6,7,8,9,10,12,15,20,25,30,40,60,100' --output-dir cosmo_reconstruct/outputs/full_hydro_hll_lbfgs_bs --cooling-stop-gradient --optimizer lbfgs --save-every 10
#!/usr/bin/env bash
#!/usr/bin/env bash
set -euo pipefail
...
XRAY_COOLING_TABLE_NPZ=${XRAY_COOLING_TABLE_NPZ:-data/nyx_cooling_table.npz}

STAGE1_ITERS=${STAGE1_ITERS:-100}
STAGE2_ITERS=${STAGE2_ITERS:-1000}
STAGE1_ADAM_LR=${STAGE1_ADAM_LR:-0.001}
STAGE2_OPTIMIZER=${STAGE2_OPTIMIZER:-lbfgs}
STAGE2_ADAM_LR=${STAGE2_ADAM_LR:-0.003}
...
STAGE2_INIT_FROM=${STAGE2_INIT_FROM:-final}      # final | iter0
STAGE2_CHECKPOINT_EVERY=${STAGE2_CHECKPOINT_EVERY:-8}

OUT_ROOT=${OUT_ROOT:-cosmo_reconstruct/outputs/BATCH_2ss_fh_128_${OBSERVABLE}}
