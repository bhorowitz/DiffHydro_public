...
TARGET_SEED=${TARGET_SEED:-315}
TARGET_SCALE=${TARGET_SCALE:-1.0}
TARGET_NOISE_SIGMA=${TARGET_NOISE_SIGMA:-0.01}
...
STAGE2_ADAM_LR=${STAGE2_ADAM_LR:-0.02}
...
OUT_ROOT=${OUT_ROOT:-cosmo_reconstruct/outputs/2s_fh_128_${OBSERVABLE}}
...
CUDA_VISIBLE_DEVICES=${GPU} python cosmo_reconstruct/run_optimize_cv0_density_full_hydro.py \
  ...
  --temp-ps-loss-weight 1.0 \
  --temp-ps-loss-nbins 32 \
  --temp-ps-loss-space log
